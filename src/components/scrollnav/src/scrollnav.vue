        ...
        <div class="yd-scrollnav-tab" :style="{color: color, backgroundColor: bgcolor}" ref="navbox">
            <div class="yd-scrollnav-unfold" :class="toggle ? 'yd-scrollnav-unfold-active' : ''">
                ...
                <ul :style="{color: currentColor, backgroundColor: bgcolor}">
                    <li :style="{color: color}" v-for="(item, i) in navList" :key="i" @click.stop="scrollContent(i)" :class="activeIndex === i ? 'yd-scrollnav-current' : ''">{{item.label}}
                    </li>
                </ul>
            </div>
            <ul class="yd-scrollnav-tab-item" ref="nav" :style="{color: currentColor, height: height}">
                <li :style="{color: color}" v-for="(item, i) in navList" :key="i" :class="activeIndex === i  ? 'yd-scrollnav-current' : ''" :ref="'navitem_' + i" @click.stop="scrollContent(i)">
                    <i></i>
                    <span>{{item.label}}</span>
                </li>
            </ul>
            <div class="yd-scrollnav-toggle" :class="toggle ? 'yd-scrollnav-toggle-active' : ''" @click="toggle = !toggle"></div>
        </div>
